...
try {
    ...
    # Open VS Code
    Write-Log "Opening VS Code in new window..." "Yellow"
    try {
        Start-Process "code" -ArgumentList "-n `"$Path`""
        Write-Log "VS Code opened successfully" "Green" $true
    } catch {
        Write-Log "Failed to open VS Code: $_. Please open it manually." "Red" $true
    }
